...
class Graphics2DInstance : public pp::Instance {
 public:
  ...
  virtual bool Init(uint32_t argc, const char* argn[], const char* argv[]) {
    nacl_io_init_ppapi(
        pp_instance(), pp::Module::Get()->get_browser_interface());
    ...
    handler.handler = TtyOutStatic;
    handler.user_data = this;
    ioctl(fd1, TIOCNACLOUTPUT, &handler);
  ...
  static ssize_t TtyOutStatic(const char* buf, size_t count, void* user_data) {
    Graphics2DInstance* instance;
    instance = static_cast<Graphics2DInstance*>(user_data);
    return instance->TtyOut(buf, count);
  }

  ssize_t TtyOut(const char* buf, size_t count) {
    const char* data = static_cast<const char*>(buf);
    std::string message = "nacl_process";
 ...
 private:
  void SetPalette(int i, uint32_t c) {
    if (i < 0 || i > 255) return;
    palette_[i] = c;
  }
  ...
  void CreatePalette() {
    for (int i = 0; i < 64; ++i) {
      // Black -> Red
      SetPalette(i, MakeColor(i * 2, 0, 0));
      SetPalette(i + 64, MakeColor(128 + i * 2, 0, 0));
      // Red -> Yellow
      SetPalette(i + 128, MakeColor(255, i * 4, 0));
      // Yellow -> White
      SetPalette(i + 192, MakeColor(255, 255, i * 4));
    }
  }
    ...
    for (int y = height - 2; y < height; ++y) {
      ...
      for (int x = 0; x < width; ++x) {
        ...
        if (span || RandUint8(1, 4) == 1) {
          ...
          SetPixel(x, y, RandUint8(128, 255));
          span--;
        } else {
          SetPixel(x, y, RandUint8(32, 96));
        }
      }
    }
  }
  ...
  void UpdateFlames() {
    ...
    for (int y = 1; y < height - 1; ++y) {
      for (int x = 1; x < width - 1; ++x) {
        int sum = 0;
        sum += GetPixel(x - 1, y - 1);
        sum += GetPixel(x + 1, y - 1);
        sum += GetPixel(x - 1, y + 0);
        sum += GetPixel(x + 1, y + 0);
        sum += GetPixel(x - 1, y + 1);
        sum += GetPixel(x + 0, y + 1);
        sum += GetPixel(x + 1, y + 1);
        SetPixel(x, y - 1, sum / 7);
      }
    }
  }
    ...
    for (int y = miny; y < maxy; ++y) {
      for (int x = minx; x < maxx; ++x) {
        if ((x - cx) * (x - cx) + (y - cy) * (y - cy) < radius * radius)
          SetPixel(x, y, RandUint8(192, 255));
      }
    }
  }
